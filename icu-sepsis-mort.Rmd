...
params <- list(booster = "gbtree", objective = "binary:logistic", 
               ...
               subsample = 0.5, colsample_bytree = 0.5, seed = 23)
```
...
Now we'll perform a grid search through the parameter space. The ranges for the
search were chosen after consulting the XGBoost manual and forums for typical
values.
...
xgb_lrn$par.vals <- list(booster = "gbtree", objective = "binary:logistic", 
                         nrounds = 100L, eval_metric = "auc", nthread = detectCores())

params <- makeParamSet(makeDiscreteParam("max_depth", values = 5:20),
                       makeDiscreteParam("gamma", values = 1:10),
                       makeDiscreteParam("min_child_weight", values = 1:10),
                       makeDiscreteParam("eta", values = seq(0.01, 0.30, .01)),
                       makeDiscreteParam("subsample", values = seq(0.1, 0.5, .1)), 
                       makeDiscreteParam("colsample_bytree", values = seq(0.1, 0.5, .1)))

resamp <- makeResampleDesc("CV", stratify = T, iters = 5L)
tune_control <- makeTuneControlGrid()
